...
def summarize_for_run(
    ...
    # Only treat as valid cache if the summary was actually generated by DeepSeek
    # (model_name starts with "deepseek:"). Rule-based summaries are re-processed.
    if not rescore_all and db is not None:
        existing = db.get_summary(item.id)
        model = existing.get("model_name") or "" if existing else ""
        if existing and existing.get("prompt_version") == PROMPT_VERSION and model.startswith("deepseek:"):
