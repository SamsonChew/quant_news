...
每天自动收集量化相关论文、论坛讨论、代码项目和订阅源内容，生成结构化中文摘要、分类分数，并输出一份不过载的中文每日量化情报报告。
...
- 多个情报分区：论文研究、论坛信号、代码与工具、策略与因子、机器学习 / 大模型、风险与执行等
- 老板视角频道：`深度学习量化未来`，专门汇总深度学习在量化里的论文、知乎、X、博客和论坛信号
...
首页 `index.html` 会展示今天和过去几天的资讯流；默认最近 7 天。每条资讯都会显示原文链接，并且每个日期、每个分类最多展示 5 篇。
...
如果要把英文论文、GitHub 描述和论坛内容自动总结成高质量中文摘要，可以配置 DeepSeek。建议用环境变量或 `.env`，不要写进代码：
...
当前先支持用本地 JSON 注入知乎和 X 消息，避免依赖不稳定的网页抓取。
...
每条内容需要包含 `source`、`source_type`、`title`、`url`、`published_at`、`text`。其中 `source_type` 可以是 `zhihu` 或 `x`。
